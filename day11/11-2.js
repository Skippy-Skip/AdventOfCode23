...
fs.readFile("day11/input 11.txt", "utf8", (err, data) => {
    ...
    arr.forEach (line => {
        ...
        if (line.replaceAll(".", "").length === 0) { //Empty line, add 999.999 to the lineCounter, to simulate the extra empty lines.
            lineCounter += 999999
        } else { 
    ...
    emptyColumns.forEach(column => { //Adjust the found index location based on the amount of empty columns, to simulate the extra empty columns.
        ...
        expandedColumns +=999999; //Adjust the original column numbers after the expansion. 
    });
